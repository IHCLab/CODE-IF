...
def parse_args():
    ...
    parser.add_argument(
        "--SRratio", type=int, default=20, help="The super-resolution ratio"
    )

    parser.add_argument("--root", type=str, default="dataset", help="data root folder")
    parser.add_argument("--test_file", type=str, default="real.txt")
...
def trainer(args):
    ...
    model.load_state_dict(
        torch.load(os.path.join(os.getcwd(), "checkpoint", "Houston.pth"))
    )
    ...
    with torch.no_grad():
        ...
        for idx2, (loadinput) in enumerate(test_loader):
            ...
            for predimg, f in zip(vResult, filPath):
                savemat(
